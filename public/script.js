...
window.startSurvey = () => {
  ...
  if (!location || !department || !tenure) {
    alert("Please select Location, Department and Tenure")
    return
  }
  ...
  localStorage.setItem("surveyLocation", location)
...
function checkLoginState() {
  ...
  const location = localStorage.getItem("surveyLocation")
...
function saveCurrentFormData() {
  ...
  if (surveyIdMatch && Object.keys(answersObject).length > 0) {
    ...
    savedResponses[surveyId] = {
      ...
      location: currentLocation, // Add location here
  ...
  if (isLastSurvey) {
    try {
      ...
      for (const responseData of allSubmissions) {
        // Ensure location is included in the response data
        const submissionData = {
          ...responseData,
          location: responseData.location || currentLocation,
        }
...
window.resetSurvey = () => {
  ...
  localStorage.removeItem("surveyLocation")
